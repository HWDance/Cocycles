...
def optimise(model,
    ...
    if len(learn_rate) == 1:
        learn_rate = learn_rate * len(model.conditioner.layers)
    
    for k in range(len(model.conditioner.layers)):
        if optimise_conditioners or (len(optimise_conditioners) > 1 and optimise_conditioners[k] == True):
            param_list = list(model.conditioner.layers[k].parameters())  # store a list
    ...
    #old_params = list(conditioner_params_list[0]['params'])  # from your old code
    #print("Old code param count:", sum(p.numel() for p in old_params))
    
    #for group in conditioner_params_list:
    #    pcount = sum(p.numel() for p in group['params'])
    #    print("Group param count:", pcount)
