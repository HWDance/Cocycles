...
def evaluate_models(
    ...
    with torch.no_grad():
        for name, (model, _) in models_dict.items():
            ...
            X_int = X*1
            X_int[:,intervention_node-1] = intervention_value
            Y_cf_model = model.cocycle(X_int, X, Y)
            diff_model = Y_cf_model - Y_model  # shape (N, p)
            print(len(Y_cf_model), len(Y_cf_true))
...
#    mask = None
    ...
    # Build architectures
    transforms = get_stock_transforms(x_dim = d-1, y_dim = 1, mask = mask)
    ...
    hyper_kwargs = [{'learn_rate': lr}] + [{'learn_rate': lr *  0.1}] * (len(cocycle_models)-1)
    ...
    metrics = evaluate_models(models, idxs, sc_fun, X, Y, seed=seed, intervention_node = d-1)
