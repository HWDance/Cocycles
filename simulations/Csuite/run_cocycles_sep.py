...
from causal_cocycle.loss import CocycleMultiLoss
...
def evaluate_models(
    ...
    with torch.no_grad():
        for name, (model, _) in models_dict.items():
            ...
            Y_cf_true = X_cf[:, 1:]
            delta_true = (Y_cf_true - Y_obs)
                
            delta_model = model.cocycle(X_cf[:, :1], X_obs[:, :1], Y_obs) - Y_obs
            rmse_cf_vals = [
                rmse(delta_model[:, j].cpu(), delta_true[:, j].cpu())
                for j in range(Y_dim)
            ]
