...
from architectures import get_stock_transforms  # Assumes this returns Zuko-compatible MAFs
import os

# === Step 1: Set up SCM ground truth for Y(x) ===

def generate_scm_data(n=250, seed=0):
    np.random.seed(seed)
    m0 = np.array([0.0, 0.0])
...
def build_models(x_dim, y_dim):
    models = []
    transforms_list = [get_stock_transforms(x_dim=x_dim, y_dim=y_dim)[2]]
    for T in transforms_list:
        models.append(ZukoCocycleModel(transforms=T))
    return models
...
def run(seed=0, n=250):
    X_raw, Y, xi = generate_scm_data(n=n, seed=seed)
    x_dim = 1
    y_dim = Y.shape[1]
    # Shuffling
    ...
    best_model, _ = validate(models, loss, X_tr, Y_tr, method="fixed", train_val_split=0.5,
                             choose_best_model="overall", retrain=True, opt_kwargs = opt_kwargs)
