...
The code stores the selected architecture as an integer from 0 to 3. To make those saved indices interpretable, this document calls the four architectures A0–A3. The definitions below are the complete search space used by the linear-chain simulation, the SCM benchmark, and the 401(k) application ([architecture factory](simulations/linear_model/architectures.py#L52-L133)). Whenever a later table states “A0–A3,” it refers to this exact four-candidate search; the architecture itself does not change between methods.

| Code index / label | Exact architecture | Network depth and width | Flow depth / spline settings | Cross-validation learning rate |
|---:|---|---|---|---:|
...
“MAF” here refers to Zuko's <code>MaskedAutoregressiveTransform</code>. Conditional cocycles apply A0–A3 to the outcome with the treatment/input as context. BGM embeds that conditional outcome MAF in a coupling transform that leaves the input coordinate unchanged ([coupling factory](simulations/linear_model/architectures.py#L135-L192)). CAREFL and CausalNF apply the MAF to the complete observed vector with zero external context ([flow construction](simulations/Csuite/run_flows_mixed.py#L193-L213)). The reported runs set <code>use_dag=False</code>, so none of these transforms receives an adjacency mask.

| Use | MAF features | Context | Interpretation |
|---|---:|---:|---|
| Experiment 8.1 linear-chain cocycle | 4 | 1 | Transform (X2, ..., X5) conditional on X1 |
| Experiment 8.1 linear-chain BGM | 4 | 1 | Transform (X2, ..., X5) conditional on X1; embed this transform in a coupling flow that leaves X1 unchanged |
| Experiment 8.3 cocycle | d - 1 | 1 | Transform (X2, ..., Xd) conditional on X1 |
| Experiment 8.3 CAREFL/CausalNF | d | 0 | Transform the complete vector (X1, ..., Xd) autoregressively |
| Experiment 8.3 BGM | d - 1 | 1 | Transform (X2, ..., Xd) conditional on X1; embed this transform in a coupling flow that leaves X1 unchanged |
...
| Likelihood-flow selection | Two-fold <code>KFold(shuffle=True)</code>; candidates minimize mean negative log likelihood with Adam, then the winner is retrained on all data. | [flow selector](causal_cocycle/causalflow_helper.py#L104-L178) |
...
|---|---|---|---|---|
| Cocycle | Discrete selector with identity map at X=0 and one unconditional scalar monotonic-affine transform at X=1; no hidden layers | CMMD-V with median-heuristic Gaussian output kernel | Adam; batch 128; 1,000 epochs; learning rate 1e-2; all 2,000 observations; no validation split | Transport 100,000 control outcomes from X=0 to X=1; training seed 0, truth seed 2026 |
| Gaussian-base causal flow | Separate scalar flow for each treatment arm. Each flow has three transforms: affine → 8-bin RQS → affine; all are unconditional (<code>hidden_features=()</code>); fixed N(0,1) base | Negative log likelihood | Adam; batch 128; 1,000 epochs; learning rate 1e-2; shuffled DataLoader | Apply both fitted inverse flows to one shared set of 100,000 base draws; effect seed 17, truth seed 2026 |
| Laplace-base causal flow | Separate scalar flow for each treatment arm. Each flow has three transforms: affine → 8-bin RQS → affine; all are unconditional (<code>hidden_features=()</code>); fixed Laplace(0,1) base | Negative log likelihood | Adam; batch 128; 1,000 epochs; learning rate 1e-3 for binary noise and 1e-2 for mixed tails; shuffled DataLoader | Apply both fitted inverse flows to one shared set of 100,000 base draws |
...
| Estimator | Conditional outcome architecture | Fitted object, base, and objective | Model selection | Optimization budget |
|---|---|---|---|---|
| Cocycle CMMD-V | 4D conditional MAF for (X2, ..., X5) given X1; candidates A0–A3 | Conditional transport map; no latent base distribution; CMMD-V with median-heuristic Gaussian output kernel | Two complementary folds; select the candidate with the lowest mean validation CMMD-V; retrain on all 1,000 observations | Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; learning rate 1e-2 for A0–A2 and 1e-3 for A3 during selection and retraining; weight decay 0; no scheduler |
| Cocycle CMMD-U | 4D conditional MAF for (X2, ..., X5) given X1; candidates A0–A3 | Conditional transport map; no latent base distribution; CMMD-U with median-heuristic Gaussian output kernel | Two complementary folds; select the candidate with the lowest mean validation CMMD-U; retrain on all 1,000 observations | Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; learning rate 1e-2 for A0–A2 and 1e-3 for A3 during selection and retraining; weight decay 0; no scheduler |
| BGM-N | 4D conditional MAF for (X2, ..., X5) given X1; candidates A0–A3; embedded in a 5D coupling flow that leaves X1 unchanged | Joint generative model; learnable factorized Normal base on all five coordinates; negative log likelihood | Two shuffled folds; select the candidate with the lowest mean validation negative log likelihood; retrain on all 1,000 observations; selection is performed separately for this base | Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; selection learning rate 1e-2 for A0–A2 and 1e-3 for A3; retraining learning rate 1e-2 for every candidate; weight decay 0; no scheduler |
| BGM-L | 4D conditional MAF for (X2, ..., X5) given X1; candidates A0–A3; embedded in a 5D coupling flow that leaves X1 unchanged | Joint generative model; learnable factorized Laplace base on all five coordinates; negative log likelihood | Two shuffled folds; select the candidate with the lowest mean validation negative log likelihood; retrain on all 1,000 observations; selection is performed separately for this base | Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; selection learning rate 1e-2 for A0–A2 and 1e-3 for A3; retraining learning rate 1e-2 for every candidate; weight decay 0; no scheduler |
| BGM-T | 4D conditional MAF for (X2, ..., X5) given X1; candidates A0–A3; embedded in a 5D coupling flow that leaves X1 unchanged | Joint generative model; learnable factorized Student-t base on all five coordinates; negative log likelihood | Two shuffled folds; select the candidate with the lowest mean validation negative log likelihood; retrain on all 1,000 observations; selection is performed separately for this base | Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; selection learning rate 1e-2 for A0–A2 and 1e-3 for A3; retraining learning rate 1e-2 for every candidate; weight decay 0; no scheduler |
...
| Optimization | Full sample; Adam; batch 128; 1,000 epochs; learning rate 1e-2; no scheduler |
...
|---|---|---|
| Cocycle | Identity anchor at treatment 0. Treatments 1 and 2 each use a separate 2D monotonic-affine MAF with one autoregressive transform and a two-hidden-layer, width-32 conditioner; CMMD-U | One architecture (no architecture search); random permutation; fixed 50:50 validation; full-data retraining; Adam; batch 128; 1,000 epochs; learning rate 1e-2 ([runner](simulations/OT/run_cocycles.py#L31-L89)) |
...
Every SCM uses N=2,000, seeds 0–9, input X1, outcomes (X2, ..., Xd), no DAG mask, and do(X1=0). Evaluation uses 100,000 paired truth draws ([launcher](simulations/Csuite/run_simcsuite_cocycles.py#L6-L25), [evaluation](simulations/Csuite/run_cocycles_mixed.py#L17-L137)).
...
The cocycle and BGM use the identical conditional outcome architecture search: a (d - 1)-dimensional MAF for (X2, ..., Xd) conditional on scalar X1, with candidates A0–A3. BGM embeds this conditional MAF in a d-dimensional coupling flow that leaves X1 unchanged. CausalNF uses A0–A3 as a joint d-dimensional autoregressive flow. CAREFL uses A0–A2 as a joint d-dimensional autoregressive flow.

| Estimator | Flow architecture | Fitted object, base, and objective | Model selection and optimization |
|---|---|---|---|
| Cocycle | (d - 1)-dimensional conditional MAF for (X2, ..., Xd) given X1; candidates A0–A3 | Conditional transport map; no latent base distribution; CMMD-V with median-heuristic Gaussian output kernel | Two complementary folds; select the candidate with the lowest mean validation CMMD-V; retrain on all 2,000 observations. Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; learning rate 1e-2 for A0–A2 and 1e-3 for A3 during selection and retraining; weight decay 0; no scheduler ([implementation](simulations/Csuite/run_cocycles_mixed.py#L140-L209)) |
| CAREFL-N/L/T | d-dimensional joint autoregressive flow with no external context; candidates A0–A2 | Joint generative model; separate learnable factorized Normal, Laplace, and Student-t bases; negative log likelihood; the runner reports each base separately | Two shuffled folds within each base; select the candidate with the lowest mean validation negative log likelihood; retrain on all 2,000 observations. Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; selection learning rate 1e-2 for A0–A1 and 1e-3 for A2; retraining learning rate 1e-2 for every candidate; weight decay 0; no scheduler ([wrapper](simulations/Csuite/run_simcsuite_carefl.py#L6-L34), [implementation](simulations/Csuite/run_flows_mixed.py#L158-L236)) |
| CausalNF-N/L/T | d-dimensional joint autoregressive flow with no external context; candidates A0–A3 | Joint generative model; separate learnable factorized Normal, Laplace, and Student-t bases; negative log likelihood; the runner reports each base separately | Two shuffled folds within each base; select the candidate with the lowest mean validation negative log likelihood; retrain on all 2,000 observations. Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; selection learning rate 1e-2 for A0–A2 and 1e-3 for A3; retraining learning rate 1e-2 for every candidate; weight decay 0; no scheduler ([wrapper](simulations/Csuite/run_simcsuite_causalnf.py#L6-L34)) |
| BGM-N/L/T | (d - 1)-dimensional conditional MAF for (X2, ..., Xd) given X1; candidates A0–A3; embedded in a d-dimensional coupling flow that leaves X1 unchanged | Joint generative model; separate learnable factorized Normal, Laplace, and Student-t bases; negative log likelihood; the runner reports each base separately | Two shuffled folds within each base; select the candidate with the lowest mean validation negative log likelihood; retrain on all 2,000 observations. Adam; batch 128; 1,000 epochs per fold and 1,000 retraining epochs; selection learning rate 1e-2 for A0–A2 and 1e-3 for A3; retraining learning rate 1e-2 for every candidate; weight decay 0; no scheduler ([implementation](simulations/Csuite/run_bgm_mixed.py#L159-L238)) |
...
| Training | Two complementary folds and full-data retraining; Adam; batch 128; 1,000 epochs in each fit; initial learning rate 1e-2 for A0–A2 and 1e-3 for A3; weight decay 1e-3; StepLR every epoch with multiplier 0.9 | [config](applications/e401k_cocycle_config.py#L1-L13) |
...
| Conditional summaries | Nadaraya–Watson regression on rank income or rank predicted Y(0); Gaussian kernel initialized at length scale 1, regularization 0; 5 folds; 1,000 iterations; learning rate 0.1; subsample 256; 500 prediction points on [0.1,0.9] | [notebook](applications/e401k-Cocycles-NF.ipynb) |
...
3. Likelihood-flow cross-validation assigns learning rate 1e-3 to the last architecture in the supplied search (A3 in the full search and A2 in CAREFL's reduced search), but final retraining uses the base learning rate 1e-2 for whichever candidate wins ([selector](causal_cocycle/causalflow_helper.py#L137-L173)).
