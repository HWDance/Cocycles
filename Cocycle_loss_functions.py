...
class loss:
    ...
    def __init__(self,loss_fn, loss_param = 0.0, kernel = [], kernel_covariates = 1, outer_subsample = []):
        """
    ...
    def JMMD_M(self,model,inputs,outputs): # no input features - this method pulls outer sum outside the norm
        """
        model: cocycle_model
        inputs : X , n x d
        outputs : Y , n x p
    ...
    def CMMD(self,model,inputs,outputs):
        """
        model: cocycle_model
        inputs : (X,X',Y') , n x 2d+p
        outputs : Y' , n x p
